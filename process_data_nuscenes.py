...
def process_data(data_path, version, output_path, val_split):
    ...
    for data_class in ['train', 'val', 'test']:
        ...
        PROPORTION_OF_DATA = 0.1 #taking only 10% of the data
        len_class = len(ns_scene_names[data_class])
        if data_class == 'val':
            len_class_a_prendre = len_class
        else:
            len_class_a_prendre = round(PROPORTION_OF_DATA * len_class)
        

        for ns_scene_name in tqdm(ns_scene_names[data_class][:len_class_a_prendre]): # de base : tqdm(ns_scene_names[data_class])
            #print('ICI :', len(ns_scene_names[data_class]), len(ns_scene_names[data_class][:5]))
