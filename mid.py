...
class MID():
    ...
    def train(self):
        for epoch in range(1, self.config.epochs + 1):
            ...
            print("PH :", self.hyperparams['prediction_horizon'])
            ...
            if epoch % self.config.eval_every == 0:
                ...
                pd.DataFrame({'value': eval_ade_batch_errors, 'metric': 'ade'}
                     ).to_csv(os.path.join("drive/MyDrive/DLAV-2023/MID/results", self.config.dataset + "_" + str(ph) + "epoch" + str(epoch) + '_ade_full.csv'))
                pd.DataFrame({'value': eval_fde_batch_errors, 'metric': 'fde', 'type': 'full'}
                     ).to_csv(os.path.join("drive/MyDrive/DLAV-2023/MID/results", self.config.dataset + "_" + str(ph) + "epoch" + str(epoch) + '_fde_full.csv'))
    ...
    def eval(self, sampling, step):
        ...
        print("EVAL", eval_ade_batch_errors.shape)
        pd.DataFrame({'value': eval_ade_batch_errors, 'metric': 'ade', 'type': 'full'}
                     ).to_csv(os.path.join("drive/MyDrive/DLAV-2023/MID/results", self.config.dataset + "_" + str(ph) + '_ade_eval.csv'))
        pd.DataFrame({'value': eval_fde_batch_errors, 'metric': 'fde', 'type': 'full'}
                     ).to_csv(os.path.join("drive/MyDrive/DLAV-2023/MID/results", self.config.dataset + "_" + str(ph) + '_fde_eval.csv'))
        ade = np.mean(eval_ade_batch_errors)
        fde = np.mean(eval_fde_batch_errors)
    ...
    def _build_dir(self):
        self.model_dir = osp.join("drive/MyDrive/DLAV-2023/MID/experiments",self.config.exp_name) #osp.join("./experiments",self.config.exp_name) 
